...
class DNSGatherApp:
    ...
    def run(self):
        ...
        try:
            ...
            dns_manager = DNSManager(dns_server, port=port, timeout=timeout,
                                     use_tcp=use_tcp)

            # Initialize DNS manager for zone discovery (may be different server)
            if zone_discovery_server != dns_server:
                discovery_manager = DNSManager(zone_discovery_server, port=port,
                                               timeout=timeout, use_tcp=use_tcp)
                self.logger.info(
                    f"Using separate server for zone discovery: "
                    f"{zone_discovery_server}"
                )
            else:
            ...
            for i, zone in enumerate(zones, 1):
                ...
                if error:
                    ...
                    self.logger.warning(
                        f"Zone transfer failed for {zone.name}: {error}"
                    )
                    print(f" [FAIL] {error}")
                else:
                    zone.transfer_status = 'Success'
                    self.logger.info(
                        f"Zone transfer successful for {zone.name}: "
    ...
    def display_summary(self, zones, output_file):
        ...
        self.logger.info(
            f"Summary: {total_zones} zones, {successful} successful, "
