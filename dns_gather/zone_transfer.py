...
class ZoneTransfer:
    ...
    def perform_axfr(self, zone_name: str) -> Tuple[List[DNSRecord], str]:
        ...
        try:
            ...
            error_message = f"Zone transfer denied or malformed: {str(exc)}"
        except dns.query.TransferError as exc:
            error_message = f"Zone transfer error: {str(exc)}"
        except dns.exception.Timeout:
            error_message = f"Zone transfer timeout for {zone_name}"
        except Exception as exc:
            error_message = f"Zone transfer failed: {str(exc)}"
        ...
        for name, node in zone.nodes.items():
            ...
            for rdataset in node.rdatasets:
                ...
                for rdata in rdataset:
                    ...
                    self._validate_hostname_match(name_str, rtype, data_str, zone_name)
    ...
    def _validate_hostname_match(self, record_name: str, record_type: str,
